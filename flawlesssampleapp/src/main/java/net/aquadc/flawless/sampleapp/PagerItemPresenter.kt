...
class PagerItemPresenter : StatelessConsumerV4FragPresenter<ParcelInt>, VisibilityStateListener {
    ...
    override fun onCreate(host: MvpV4Fragment<ParcelInt>, arg: ParcelInt, state: ParcelUnit?) {
        host.addViewFirstShownListener {
            Toast.makeText(it.activity, "Fragment #${arg.value} should load data now.", Toast.LENGTH_SHORT).show()
        }
    }

    override fun createView(host: MvpV4Fragment<ParcelInt>, parent: ViewGroup?, arg: ParcelInt, state: ParcelUnit?): View =
    ...
    override fun onViewCreated(host: MvpV4Fragment<ParcelInt>, view: View, arg: ParcelInt, state: ParcelUnit?) {
